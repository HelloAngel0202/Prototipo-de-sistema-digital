...
const LenderConditions = () => {
    ...
    useEffect(() => {
        const getClientRequest = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/users/get-client-request?request_id=${request_id}`);
    ...
    useEffect(() => {
        const calculateLoanAndCosts = () => {
            ...
            if (principal > 0 && annualInterest > 0 && periods > 0) {
                ...
                const fee = (principal * monthlyRate * Math.pow(1 + monthlyRate, periods)) /
    ...
    const handleSubmit = async (e) => {
        ...
        const dataToSave = {
            ...
            estimated_fee_amount: Number(formData.estimated_fee_amount.replace(/\D/g, "")),
        ...
        try {
            await axios.post('http://localhost:3001/users/lender-conditions', dataToSave);
            Swal.fire({
                title: "Enviado",
            ...
            <div className="form-card">
                ...
                <form className="lender-conditions-form" onSubmit={handleSubmit}>
                    <div className="grid-container">
                        ...
                        <label className="field-label">
                            Tipo de Interés
                            <select className="form-select" name="interest_type" value={formData.interest_type} onChange={handleChange}>
                        ...
                        <label className="field-label">
                            Frecuencia de Pago
                            <select className="form-select" name="payment_frequency" value={formData.payment_frequency} onChange={handleChange}>
                    ...
                    <button className="submit-button" type="submit">Enviar Oferta al Cliente</button>
                </form>
            </div>
        </div>
    );
